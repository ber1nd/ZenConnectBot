...
class ZenQuest:
    ...
    async def handle_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Handle unfeasible actions and failure actions (unchanged)
        ...
        # Check for moral consequences (unchanged)
        ...
        next_scene = await self.generate_next_scene(
            ...
            quest_goal=self.quest_goal[user_id]
        )
    ...
    async def end_pvp(self, update: Update, context: ContextTypes.DEFAULT_TYPE, victory: bool):
        user_id = update.effective_user.id
        self.in_combat[user_id] = False

        # Generate a consequence based on the battle outcome
        consequence = await self.generate_pvp_consequence(victory)
        
        await update.message.reply_text(consequence)

        # Update the quest state based on the battle outcome
        if victory:
            self.player_karma[user_id] = min(100, self.player_karma.get(user_id, 0) + 10)
            next_scene = await self.generate_next_scene(update, self.current_scene[user_id], "victory in PvP", self.quest_state[user_id], self.quest_goal[user_id])
        else:
            self.player_karma[user_id] = max(0, self.player_karma.get(user_id, 0) - 10)
            next_scene = await self.generate_next_scene(update, self.current_scene[user_id], "defeat in PvP", self.quest_state[user_id], self.quest_goal[user_id])
