...
class ZenQuest:
    ...
    async def progress_story(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_input: str, user_id: int = None):
        ...
        try:
            ...
            # End quest if HP is depleted or karma is too low
    ...
    async def end_pvp_battle(self, context: ContextTypes.DEFAULT_TYPE, user_id: int, victory: bool, battle_id: int):
        try:
            ...
            if user_id != 7283636452:  # Only update scene for real players, not the bot
                ...
                if not victory:
                    await self.end_quest(None, context, victory=False, reason="You have been defeated in combat.", user_id=user_id)
                else:
                    # Continue the quest after a win
    ...
    async def surrender(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if battle:
                ...
                """
                consequence = await self.generate_response(consequence_prompt)
                ...
                # If the player's HP or Karma drops too low, end the quest
    ...
    async def end_quest(self, update: Update, context: ContextTypes.DEFAULT_TYPE, victory: bool, reason: str, user_id: int = None):
        ...
        # Generate a Zen-like conclusion based on victory or defeat
        ...
        # Adjust Zen points depending on victory or defeat
        ...
        # Clean up the user's quest data
    ...
    async def handle_self_harm(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_input: str):
        ...
        # End the quest if HP drops to 0 or below
        if self.player_hp[user_id] <= 0:
    ...
    # Method to update the current quest state based on the player's progress
    ...
    # Utility to process the scene into a description and choices
